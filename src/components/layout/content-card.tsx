...
const LoadingCard: React.FC<{
  ...
  if (variant === "news") {
    return (
      <div
        className={`w-full h-full flex items-center justify-start ${className}`}
      >
  ...
  if (variant === "project") {
    return (
      <div
        className={`border border-gray-200 dark:border-zinc-800 rounded-lg p-6 ${className}`}
      >
  ...
  if (variant === "project") {
    return (
      ...
      >
        ...
        <h3 className="font-medium text-gray-800 dark:text-zinc-100 mb-2">
          加载失败
        </h3>
        <div className="text-sm text-gray-500">请稍后重试</div>
      </div>
...
export const ContentCard: React.FC<ContentCardProps> = ({
  ...
  useEffect(() => {
    if (!enableISR || !data?.lastUpdated) {
      if (process.env.NODE_ENV === "development" && !data?.lastUpdated) {
        console.log("[ContentCard] No lastUpdated data for ISR check:", {
          title: data?.title,
          variant,
        });
      }
  ...
  if (isLoading || isRefreshing) {
    if (process.env.NODE_ENV === "development") {
      console.log("[ContentCard] Showing loading state:", {
        variant,
        isLoading,
        isRefreshing,
      });
  ...
  if (!data || !data.title) {
    if (process.env.NODE_ENV === "development") {
      console.warn("[ContentCard] No data or title:", {
        variant,
        hasData: !!data,
        title: data?.title,
        cacheStatus,
  ...
  if (variant === "recommend") {
    return (
      <div
        className={`pt-10 cursor-pointer ${className}`}
        onClick={handleClick}
      >
        ...
        <div className="pt-1">
          <div className="text-xl font-bold py-3">{title}</div>
